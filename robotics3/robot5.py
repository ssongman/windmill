...
##[History]###################################
# robot3_auto2.py
# 2016.10.20 : add autopilot functon
# 
# robot4_2.py
# 2016.10.25 : install infrared(analog chip)
# 
# robot5.py
# 2016.11.01 : add radar funtion
####################################################################################################
...
ULTR_PCA_PIN = 7
# Configure min and max servo pulse lengths
ULTR_PULSE_MIN =  210   # Min pulse length out of 4096
...
def RadarWin():
    print('Radar window started...')
    
    winRadar = Toplevel(top)
    winRadar.title('Radar Window')
    winRadar.geometry("1500x800+200+50")
    
    CANVAS_WIDTH  = 1500
    CANVAS_HEIGHT = 800
    
    C = Canvas(winRadar, width=CANVAS_WIDTH, height=CANVAS_HEIGHT, bg="black")
    C.pack()

    # create Radar objects
    from radar5 import Radar
...
def forward_distance_chk():
    ...
    while True:
        ...
        distance_f = get_distance_red_f()   # infrared
...
def reverse_distance_chk():
    ...
    while True:
        ...
        distance_b = get_distance_red_b()   # infrared
...
def KeyPress_T(event):
    ...
    distance = get_distance()
    distance_f = get_distance_red_f()
    distance_b = get_distance_red_b()
...
def get_distance_red_f():
    ...
    if volts >= 0 and volts <= 530 and volts <> 11:
