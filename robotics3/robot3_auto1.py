...
ULTR_PCA_PIN = 7
# Configure min and max servo pulse lengths
ULTR_PULSE_MIN =  250  # Min pulse length out of 4096
ULTR_PULSE_MAX =  950  # Max pulse length out of 4096
ULTR_PULSE_MID =  570  # MID

ULTR_MOVING_PULSE = 50

ULTRA_SERCH_ANGLES = [250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900]
#                     ---------  _____________  ------------------  _____________  --------
#                     LEFT           LEFT-MID       MIDDLE         MID-RIGHT       RIGHT 
...
def SerchDirection():
    ...
    intLMid  = min(DistPerAngles[ 2: 5])
    intMid   = min(DistPerAngles[ 5: 9])
    intMidR  = min(DistPerAngles[ 9:12])
    intRight = min(DistPerAngles[12:14])
...
def get_distance():
    ...
    strDist = "Distance : " + str(distance) + "cm"
    print(strDist)
    strDis.set(strDist)
    return distance
...
speed_scale = Scale(frmSpeed, length=300, width=20, sliderlength=10, from_=0, to=100, orient=HORIZONTAL, tickinterval=20, command=changeSpeed)  # Create a scale
speed_scale.set(50)
...
# Auto Drive Mode
# =============================================================================
fromAuto = Frame(top)  # bd=3, relief=RAISED, width= 150, height=100)
DriveMode=0            # 0:MenualMode,  1: AutoMode
def AutoStart():
    global DriveMode
    global run_direct
    strModetxt.set("Automatic Drvie Mode...")
    #lbAuto.config(fg="red", font=30)     # python 3.x ...
    ...
    ## forward wait...
    #while (DriveMode == 1 and run_direct == 'FORWARD') :
    #   strDirection.set("Direction: forward")
    #   
    ## End while
    #
    ## SerchDirection
...
def AutoStop():
    global DriveMode
    strModetxt.set("")
    #lbAuto.config(fg="black", font=10)
    
    DriveMode=0

strModetxt  =StringVar()
strDirection=StringVar()
Label(fromAuto, text='[Auto Drive Mode]', fg='black', font=20).grid(row=0, column=0, sticky='W', columnspan=2)
Button(fromAuto, text="Start", font=20, width=10, height=3, command=AutoStart).grid(row=1, column=0, sticky=W+E+N+S, padx=5, pady=5)
Button(fromAuto, text="Stop" , font=20, width=10, height=3, command=AutoStop ).grid(row=1, column=1, sticky=W+E+N+S, padx=5, pady=5)
lbAuto = Label(fromAuto, text='Drive Mode : Menual', font=30, fg='red', textvariable=strModetxt  ).grid(row=2, column=0, columnspan=2)
lbDire = Label(fromAuto, text='Direction: forward' , font=10, fg='red', textvariable=strDirection).grid(row=3, column=0, columnspan=2)
...
btnGetDis = Button(frmUltraSonicBtn, height=3, text='get_distance(T)', bd=8, fg="black")
lbDis     = Label (frmUltraSonicBtn, textvariable=strDis, text='Distance : 9999cm', fg="red",font=20, justify=LEFT, bd=3)
btnSerch  = Button(frmUltraSonicBtn, text="SerchDirection", font=20, width=10, height=3, command=SerchDirection)
lbDir     = Label (frmUltraSonicBtn, textvariable=strDir, text='Dircection : MID', fg="red",font=20, justify=LEFT, bd=3)
...
btnGetDis.grid(row=1, column=0) #, sticky='W')
lbDis    .grid(row=1, column=1) #, sticky='W')
btnSerch .grid(row=2, column=0, padx=5, pady=5) #, sticky='W')
lbDir    .grid(row=2, column=1) #, sticky='W')
...
fromAuto        .grid(row=4, column=0, sticky='W')
frmUltraSonicBtn.grid(row=4, column=2, sticky='W')

#top.geometry("1000x500")
top.geometry("800x700+400+50")
top.mainloop()
