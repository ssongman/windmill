...
def main():
    ...
    while cv2.waitKey(1) != 27 and capWebcam.isOpened():                # until the Esc key is pressed or webcam connection is lost
        ...
        ## rotateimage 180 degree
        rows,cols = imgOriginal.shape[:2]
        M = cv2.getRotationMatrix2D((cols/2,rows/2),180,1)
        imgOriginal = cv2.warpAffine(imgOriginal,M,(cols,rows))
        ...
        if circles is not None:                     # this line is necessary to keep program from crashing on next line if no circles were found
            ...
            if x < intXFrameCenter and panServoPosition >= 10:
            ...
            if y < intYFrameCenter and tiltServoPosition >= 0:    # 62
            ...
            print "panServoPosition = " + str(panServoPosition) + ",  tiltServoPosition = " + str(tiltServoPosition) + " " + direct_x + direct_y
